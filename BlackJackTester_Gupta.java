        ...
        while (state != State.EXIT)
        {
            switch(state)
            {
                ...
                case ROOM_SELECT: //MOHNISH
                    ...
                    /**
                     //OI OI OI Mongo oʻer ʻere (construct object
                     if (isDefault)
                     {
                     ...
                     {
                        //@ author Mohnish (pls)
                        //roomName = [some mongo thing];
                        //moneys = [some mongo thing];

                        Blackjack game = new Blackjack(roomName, moneys);
                        game.game();
                            //buttons for moves (hit, stand, etc) in game function game()
                    ...
                    break;
                case PLAY_GAME: //NICOLE //what
                    System.out.println("You are in play_game screen");
                    //select room
                    Scanner scannr7 = new Scanner(System.in); //no need anymore?
                    state1 = scannr7.nextLine();              //maybe? idk
...
class Blackjack
{
    ...
    public void game()
    {
        //buttons to add: HIT   STAND   DOUBLE_DOWN     SPLIT   REMATCH (no to "exit")
        //              hitter  stander   doubler      splitter  play_again
        //                                  -             -           -
        //uinputs to add: AMT_BET
                        ...
                        for (Hand hand : player.getHandArray())
                        {
                            ...
                            if (hand.get(0).getValue() == hand.get(1).getValue() && count == 1 && player.isBlackjack(hand) == false)
                            {
                                ...
                                if (count == 1)
                                {
                                    ...
                                    while (splitter != "SPLIT" && splitter != "NO")
                                    {
                                        System.out.println("Please select SPLIT or NO");
                                        response = scanner.nextLine(); //@SHAIVYA HOW RECEIVE INFO FROM BUTTONS
                                    }
         ...
         **/
            ...
            else
            {
                ...
                */
                ...
                while (play_again != "REMATCH" && play_again != "NO")
                {
                    System.out.println("Please select REMATCH or NO");
                    response = scanner.nextLine();
                }
...
class Player
{
    ...
    public boolean getUserChoice()
    {
        ...
        */
        ...
        String hitter = "";
        System.out.println("Would you like to get a card (hit)?");

        while (hitter != "HIT" && hitter != "NO")
        {
            System.out.println("Please select HIT or NO");
            response = scanner.nextLine();
        }
    ...
    public boolean DoublingDown(int i)
    {
        ...
        */
        ...
        while (doubler != "DOUBLE DOWN" && doubler != "NO")
        {
            System.out.println("Please select DOUBLE DOWN or NO");
            response = scannering.nextLine();
        }

        if (doubler == "DOUBLE DOWN")
        {
            money -= pot[i];
            pot[i] *= 2;
            return true;
