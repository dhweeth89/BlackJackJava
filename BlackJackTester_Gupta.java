...
class Login //oer here
{
    ...
    String logpass = pw.nextLine();

    boolean loggedIn = db.auth( <username>, passwordPrompt() );
    if (loggedIn)
    {
        main(String[] args);
    }
    ...
    /**
    //search for first matching username since should not allow for duplicates
    Document = collection.find(eq("username", loguser)).first();
    //System.out.println(myDoc.toJson());
    if (Document.logpass == user.password)
    {
        //
